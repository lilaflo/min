var tagIndex = {
  ...
  getAllTagsRanked: function (page) {
    ...
    for (var tagName in scores) {
      if (tokens.includes(tokenize(tagName)[0])) {
        scores[tagName] *= 1.5
      }
      if (contributingDocs[tagName] > 1 && contributingTerms[tagName] > 1) {
        scoresArr.push({ tag: tagName, value: scores[tagName] })
      } else {
