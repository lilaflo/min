/* eslint-disable no-unused-vars, no-path-concat */
...
function sendIPCToWindow (window, action, data) {
  ...
  if (window && getWindowWebContents(window).isLoadingMainFrame()) {
    ...
    var newWin = createWindow()
    getWindowWebContents(newWin).once('did-finish-load', function () {
      getWindowWebContents(newWin).send(action, data || {})
    })
  }
}
